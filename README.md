...
The second way: the meat stays on the board. You pick up the knife and move it. The meat is fixed. The blade is what travels.
...
The roots of your equation are exactly where the line intercepts the fixed curve.

That's the Fixed Curve Intercept Method.

## What Changes When You Hold It This Way

When you fix the curve and move the line, things that were hidden become visible.
...
- Line intercepts the curve at two points → two real roots → Δ > 0
- Line touches the curve at exactly one point → one repeated root → Δ = 0
- Line misses the curve entirely → no real roots → Δ < 0
...
When the line misses the curve, the vertical distance between them at the axis of symmetry is the imaginary part of the complex roots made visible. The roots didn't disappear — they moved off the real line, and you can measure how far.
...
For degree n, fix `y = xⁿ`. Move a degree n−1 curve. Read the intercepts.

The maximum number of intercepts is always n. That is why a degree-n polynomial has at most n roots. Not because of algebra — because a degree-n curve and a degree-(n−1) curve can only meet so many times. The geometry states the bound before the algebra does.


## What This Repository Is

This is an open collection of explorations, visualisations, and notes built around the Fixed Curve Intercept Method.
...
- **Interactive visualisations** — the fixed curve, the moving line, the roots as intercepts
...
> Fixed Curve Intercept Method: fix the pure power curve, move everything else against it.  
...
Welcome to the Fixed Curve Intercept Method.
